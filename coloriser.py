...
def colorize_image(net,image_filename=None, cv2_frame=None):
    ...
    lab = cv2.cvtColor(scaled , cv2.COLOR_BGR2LAB) #converts from bgr to lab

    resized = cv2.resize(lab , (224,224))
    L = cv2.split(resized)[0] #took out the colour space so now image only has black and white part
    L -= 50 
    ...
    cv2.imwrite('./static/color_pic.jpg',colorized)
